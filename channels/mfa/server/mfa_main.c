...
#define CURRENT_VERSION MFA_VERSION_2
...
void pf_mfa_wait_for_token_expired_thread(MfaServerContext* context)
{
	...
	if (mfa->status != MFA_STATUS_AUTH_SUCCESS)
	{
		/* exp_thread is not set, client probably didn't provide a token */
		WLog_INFO(TAG,
		          "pf_mfa_wait_for_token_expired_thread(): mfa not initialized, no need to wait");
		return;
	}
...
static MFA_STATUS mfa_wait_for_auth(MfaServerContext* context, DWORD timeout)
{
	DWORD status;
	MfaServerPrivate* mfa = (MfaServerPrivate*)context->handle;

	status = WaitForSingleObject(mfa->auth_status, timeout);

	WLog_INFO(TAG, "mfa_wait_for_auth: WaitForMultipleObjects returned 0x%x", status);
	...
	if (status == WAIT_TIMEOUT)
	{
		WLog_WARN(TAG, "pf_client_wait_for_mfa: timeout passed.");
