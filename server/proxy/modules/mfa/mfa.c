...
static BOOL mfa_pre_connect(moduleOperations* module, rdpContext* context)
{
	...
	rdpContext* server = (rdpContext*)pc->pdata->ps;

	MfaServerContext* mfa = module->GetSessionData(module, server);
	if (mfa == NULL)
	{
	...
	mfa_st = mfa->WaitForAuth(mfa, 20 * 1000);
	switch (mfa_st)
	{
...
{
	struct token_response_t resp = { 0 };
	...
	if (resp.is_valid)
	{
		context->SetTokenInfo(context, resp.exp, resp.nonce);
		return TRUE;
	}
...
static BOOL mfa_config_load()
{
	...
	config->mfa_adfs_base_url = _strdup(CONFIG_GET_STR(ini, "MFA", "AdfsBaseUrl"));
	config->mfa_audience = _strdup(CONFIG_GET_STR(ini, "MFA", "Audience"));
	config->insecure_ssl = CONFIG_GET_BOOL(ini, "MFA", "InsecureSSL");
